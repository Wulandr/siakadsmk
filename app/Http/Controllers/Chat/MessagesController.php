...
class MessagesController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:chat_create', ['only' => 'postSendMessage']);
    }
    /**
    ...
    public function getLoadLatestMessages(Request $request)
    {
        ...
        $mes = Message::all()->where('from_user', Auth::user()->id)->where('to_user', $request->user_id);

        $return = [];
        foreach ($messages->reverse() as $message) {
            $return[] = view('dashboard.chat.message-line')->with('message', $message)->render();
        }
        return response()->json(['state' => 1, 'messages' => $return, 'mes' => $mes]);
    }
